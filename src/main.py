...
def opc_log(val):
    while(val == True):
        banner = header.figlet_format("Cifrado Cesar")
        ...
        if eleccion == 1:   
            ...
            #Se envia el mensaje y la llave para encriptar que en este caso es 3
            enc_msj(mensaje, 3)
        elif eleccion == 2:
...
def enc_msj(mensaje, llave_cesar):  
    ...
    print(colored("Mensaje encriptado > {}\n Se genero el archivo cifrado.txt".format(resultado), "green", attrs=["bold"]))
...
def main():
    while(True):
        clear()
        banner = header.figlet_format("Bienvenido PIA062")
        print(colored(banner.rstrip("\n"), "red", attrs=["bold"]))        
        print(colored("1 -- Registro", "blue", attrs=["bold"]))
        print(colored("2 -- Login", "blue", attrs=["bold"]))
        print(colored("3 -- Reinicai", "blue", attrs=["bold"]))
        print(colored("4 -- Salir", "blue", attrs=["bold"]))
        opc = int(input(colored(" Selecciona una opción > ", "green", attrs=["bold"])))
        #Si se elige 2 manda al Registro
        if opc == 2:            
            while(True):
                certificado: bytes = input((colored("Certificado (usuario.cer) > ", "blue", attrs=["bold"])))
                privada: bytes = input((colored("Clave privada (usuario.key) > ", "blue", attrs=["bold"])))
                password = input((colored("Contraseña de la clave privada > ", "blue", attrs=["bold"])))
                if not certificado or not privada or not password:
                    print(colored("Todos los campos son obligatoríos", "red", attrs=["bold"]))
                else:
